...
with st.expander("新規番組を登録", expanded=False):
    ...
    new_program_title = st.text_input(
        "番組名", placeholder="例: オールナイトニッポン", key="new_program_title"
    )

    col1, col2 = st.columns(2)
    with col1:
        new_program_email = st.text_input(
            "投稿用メールアドレス",
            placeholder="example@radio.com",
            key="new_program_email",
        )
    with col2:
        new_program_schedule = st.text_input(
            "放送スケジュール",
            placeholder="例: 月〜金 6:00-10:00",
            key="new_program_schedule",
        )

    # パーソナリティ選択
    personality_names = [p["name"] for p in personalities]
    selected_personalities = st.multiselect(
        "パーソナリティ", personality_names, key="selected_personalities"
    )

    col1, col2, col3 = st.columns([1, 1, 4])
    with col1:
        if st.button(
            "登録", type="primary", use_container_width=True, key="create_program_btn"
        ):
            if new_program_title and new_program_email:
                try:
                    ...
                    personality_ids = [
                        p["id"]
                        for p in personalities
                        if p["name"] in selected_personalities
                    ]
...
try:
    ...
    if selected_personality != "全てのパーソナリティ" and personalities:
        ...
        for p in personalities:
            display_name = f"{p['name']}" + (
                f" ({p['nickname']})" if p.get("nickname") else ""
            )
    ...
    programs = api_client.get_programs(
        personality_id=personality_id, search=program_search if program_search else None
    )
    ...
    if not programs:
        ...
        for program in programs:
            with st.expander(f"{program['title']}", expanded=False):
                ...
                with col1:
                    st.markdown(
                        f"**メールアドレス:** {program.get('email_address', 'N/A')}"
                    )
                    st.markdown(
                        f"**放送スケジュール:** {program.get('broadcast_schedule', 'N/A')}"
                    )

                    # パーソナリティ表示
                    personality_names = [
                        p["name"] for p in program.get("personalities", [])
                    ]
                    if personality_names:
                        st.markdown(
                            f"**パーソナリティ:** {', '.join(personality_names)}"
                        )

                with col2:
                    if st.button(
                        "編集",
                        key=f"edit_program_{program['id']}",
                ...
                if not corners:
                    ...
                    for corner in corners:
                        ...
                        with col2:
                            if st.button(
                                "削除",
                                key=f"delete_corner_{corner['id']}",
                                use_container_width=True,
                            ):
                                try:
                ...
                if st.button(
                    "コーナーを追加",
                    key=f"add_corner_btn_{program['id']}",
                    use_container_width=True,
                ):
                ...
                if st.session_state[add_corner_key]:
                    ...
                    with col1:
                        if st.button(
                            "追加",
                            key=f"submit_corner_{program['id']}",
                            type="primary",
                            use_container_width=True,
                        ):
                            if new_corner_title and new_corner_desc:
                                try:
                                    ...
                                    st.success(
                                        f"コーナー「{new_corner_title}」を追加しました！"
                                    )
                                    st.session_state[add_corner_key] = False
                                    st.rerun()
                                except Exception as e:
                    ...
                    with col2:
                        if st.button(
                            "キャンセル",
                            key=f"cancel_corner_{program['id']}",
                            use_container_width=True,
                        ):
                            st.session_state[add_corner_key] = False
                            st.rerun()
