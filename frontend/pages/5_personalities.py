...
with st.expander("新規パーソナリティを登録", expanded=False):
    new_name = st.text_input(
        "名前", placeholder="例: 山田太郎", key="new_personality_name"
    )
    new_nickname = st.text_input(
        "ニックネーム（任意）",
        placeholder="例: たろちゃん",
        key="new_personality_nickname",
    )
    ...
    with col1:
        if st.button(
            ...
            key="create_personality_btn",
        ):
            if new_name:
                try:
                    api_client.create_personality(
                        name=new_name, nickname=new_nickname if new_nickname else None
                    )
...
try:
    ...
    if not personalities:
        st.info(
            "パーソナリティが登録されていません。新しいパーソナリティを登録してください。"
        )
    else:
        # 検索フィルター
        ...
        if search_query:
            filtered_personalities = [
                ...
                or (p.get("nickname") and search_query.lower() in p["nickname"].lower())
            ]
            ...
            with st.container():
                ...
                with col2:
                    ...
                    with col_edit:
                        if st.button(
                            "✏️",
                            key=f"edit_btn_{personality['id']}",
                            use_container_width=True,
                            help="編集",
                        ):
                    ...
                    with col_delete:
                        if st.button(
                            "🗑️",
                            key=f"delete_btn_{personality['id']}",
                            use_container_width=True,
                            help="削除",
                        ):
                            st.session_state[f"confirm_delete_{personality['id']}"] = (
                                True
                            )
                            st.rerun()
                ...
                if st.session_state.get(f"confirm_delete_{personality['id']}", False):
                    with st.container():
                        ...
                        with col_yes:
                            if st.button(
                                "削除する",
                                key=f"confirm_yes_{personality['id']}",
                                type="primary",
                                use_container_width=True,
                            ):
                                try:
                                    ...
                                    del st.session_state[
                                        f"confirm_delete_{personality['id']}"
                                    ]
                                    st.rerun()
                                except Exception as e:
                                    st.error(f"削除エラー: {e}")
                        ...
                        with col_no:
                            if st.button(
                                "キャンセル",
                                key=f"confirm_no_{personality['id']}",
                                use_container_width=True,
                            ):
                                del st.session_state[
                                    f"confirm_delete_{personality['id']}"
